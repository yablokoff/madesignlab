...
var nav;
var imageWidths = [2040,461,1500,1742,434,600];
var imageHeights = [1186,895,2294,1131,883,600];
...
var captionRightEdges = [0,231,750,605,217,200]; //от физической середины картинки до правого края видимого изображения (монитор)

//var captionBottomEdges = [0,580,545,390,642,580,580,545,545];

var captionTopEdges = [0,402,1147,440,395,230]; //от физической середины картинки до верхнего края видимого изображения (монитор)

// Padding added to the bottom in the coordinate system of the slide divs
var applyBottomPaddingToCaption = [false,false,true,false,false];
...
$(document).ready(function($) {
	...
	var currentMoment = moment.tz(currentTime, "America/Los_Angeles").format("LT");
...
layoutCaptions = function() {
	...
	$(".caption").each(function(i, val) {
		...
		var y = ((viewportHeight / 2.0) - (captionTopEdges[i] * scale));
...
selectTabIndex = function(i, animated) {
	...
	for (var ind = 0, len = tabs.length; ind < len; ind++) {
		if (tabs[ind].classList.contains("_active")) {
			tabs[ind].classList.remove("_active");
		}
	}
